...
struct _WMI {
    _WMI() {
        auto res = CoInitializeSecurity(NULL, -1, NULL, NULL, RPC_C_AUTHN_LEVEL_DEFAULT, RPC_C_IMP_LEVEL_IMPERSONATE, NULL, EOAC_NONE, NULL);
...
std::vector<CPU> get_component() {
    _WMI wmi;
    const std::wstring query_string(L"SELECT Name, Manufacturer, NumberOfCores, NumberOfLogicalProcessors, MaxClockSpeed, L2CacheSize, L3CacheSize FROM Win32_Processor");
    bool success = wmi.execute_query(query_string);
    if (!success) {
        return {};
    }
