...
std::map<uint32_t, std::vector<std::string>> getDiskToVolumeMap() {
  ...
  while (*currentDrive) {
    ...
    if (driveType == DRIVE_FIXED || driveType == DRIVE_REMOVABLE) {
      ...
      HANDLE hVolume = CreateFileA(volumePath.c_str(), 0, FILE_SHARE_READ | FILE_SHARE_WRITE,
                                   nullptr, OPEN_EXISTING, 0, nullptr);

      if (hVolume != INVALID_HANDLE_VALUE) {
        STORAGE_DEVICE_NUMBER sdn;
        DWORD bytesReturned;
        if (DeviceIoControl(hVolume, IOCTL_STORAGE_GET_DEVICE_NUMBER, nullptr, 0,
  ...
  for (uint32_t i = 0; i < 16; ++i) {
    ...
    if (DeviceIoControl(hDevice, IOCTL_STORAGE_QUERY_PROPERTY, &query, sizeof(query),
      ...
      if (desc->VendorIdOffset) {
        ...
        std::transform(disk._model.begin(), disk._model.end(), upper_model.begin(), [](const char c) { return static_cast<char>(std::toupper(static_cast<int>(c))); });
