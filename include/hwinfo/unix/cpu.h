 ...
 */
std::map<std::uint32_t, std::map<std::uint32_t, std::map<std::string, std::string>>> parse_cpuinfo_file(const std::string& data);

/**
 * parses a sockets map (generated by e.g. parse_cpuinfo_file) into a vector of CPU instances
 */
std::map<std::uint32_t, unix_os::cpu::CPU> parse_cpus(const std::map<std::uint32_t, std::map<std::uint32_t, std::map<std::string, std::string>>>& sockets);
